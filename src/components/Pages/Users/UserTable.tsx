...
import { State } from '@/types/prisma';
...
function UserTable({ filteredData }: { filteredData: User[] }) {
    ...
    const toggleUserState = async (id: number) => {
        ...
        if (user) {
            ...
            await updateUser(id, { ...user, state: newState as State });
        }
    }
                ...
                <Table stickyHeader sx={{ minWidth: 650 }} aria-label="a dense table">
                    ...
                    <TableBody>
                        {(rowsPerPage > 0 ? filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : filteredData).map((row, index) => (
                            <TableRow
                                onDoubleClick={() => router.push(`/admin/userRegister/${row?.id ?? -1}`)}
                                key={row.id}
                                sx={{
                            ...
                            >
                                ...
                                <TableCell >
                                    <div className='flex justify-center gap-2'>
                                        <Button
                                            onClick={() => router.push(`/admin/userRegister/${row?.id ?? -1}`)}
